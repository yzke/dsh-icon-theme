...
发现使用 `entriesOfSlot()` 的渲染胜出项，而不是包含被高优先级覆盖项的原始
`entries()` 账本。只有胜出项能进入 DOM 相关联步骤，避免一个已被遮蔽的同 ID 注册让
设置整面误判为数量不一致。

设置适配器只接受唯一候选：`role="dialog"` 的 `aria-labelledby` 必须指向弹窗内包含
`data-slot="settings.header"` 的标题节点，导航按钮数量还必须与胜出项完全一致。形状相似
的其他弹窗或两个同时满足条件的候选都会拒绝处理。两个适配器只观察会影响契约的有限
属性集、子节点和文字变化；侧边栏的文字与 class 观察严格限定在实际 footer slot 内，全局
观察器只筛选 slot 的挂载、卸载或 `data-slot` 变化，不会让聊天流式文本触发完整扫描。宿主
原地重建按钮内容时，还会验证自有 glyph、managed 标记与 mask 并安全重建，避免属性残留
却图标空白或原图标与自有图标同时出现。
...
1. 用户对稳定目标键的手动覆盖。
2. 已审核、静态打包的插件精确图标。
3. 在“优先原图”策略下保留 DSH/插件现有图标；“替换通用回退”策略下只保留非通用原图标。
4. 稳定 ID 精确预设。
